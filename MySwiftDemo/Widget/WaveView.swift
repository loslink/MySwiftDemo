...
class WaveView: UIView {
    ...
    // 真实波浪颜色
    var realWaveColor: UIColor = UIColor.blue
    // 阴影波浪颜色
    var maskWaveColor: UIColor = UIColor.black
    // 波浪位置（默认是在下方）
...
//        displayLink!.add(to: RunLoop.current, forMode: RunLoop.Mode.common)
    ...
    // 定时器响应（每一帧都会调用一次）
    @objc func wave() {
        let duration=displayLink?.duration //屏幕刷新帧之间间隔时间(秒)
        let frameInterval=displayLink?.frameInterval //间隔几帧回调一次
        let perSelectTime = duration!*Double(frameInterval!) //间隔多久回调这个方法一次
        let actualFramesPerSecond = 1 / (displayLink!.targetTimestamp - displayLink!.timestamp) //每秒刷新多少帧
        
//        print("duration:\(duration)")
//        print("frameInterval:\(frameInterval)")
//        print("perSelectTime:\(perSelectTime)")
//        print("actualFramesPerSecond:\(actualFramesPerSecond)")
        
//        let width=UIScreen.main.bounds.width
//        print("one round Time:\(perSelectTime*Double(width/waveSpeed))")
